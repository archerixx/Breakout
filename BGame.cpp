...
bool BGame::loadBackgroundMedia(const char* path)
{
	...
	if (!gameOverBackground->loadFromFile("Breakout_Media/gameover_background.png"))
	{
		std::cout << "\nFailed to load game over texture image!\n";
		success = false;
	}
	//Load menu text
	if (!menuText->loadFromFile("Breakout_Media/menu_text.png"))
	{
		std::cout << "\nFailed to load game over texture image!\n";
		success = false;
	}
...
bool BGame::loadBallMedia()
{
	...
	if (!ballTexture->loadFromFile("Breakout_Media/Ball.png"))
	{
...
bool BGame::loadScoreAndLivesMedia(int gameScore, int livesLeft)
{
	...
	//Open the font
	baseFont = TTF_OpenFont("Breakout_Media/MAGNETOB.ttf", 26);
	if (baseFont == NULL)
	{
